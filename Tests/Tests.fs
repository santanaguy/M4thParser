...
let ``InferMultiplications infers correctly functions``()=
    ...
    test <@output = [one; timesNotParsed; Sqrt(Group[one]); timesNotParsed; one; timesNotParsed; Sqrt(Group[one]);]@>
...
let ``buildExpressions adds arguments to sqrt``() = 
    ...
    test <@outpupt = [one; plusNotParsed; Sqrt(Group[one; xVariable]); Sqrt(Group([one]))]@>
